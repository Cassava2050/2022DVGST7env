...
sel_file_use
trial2_tidy = read.csv(here::here("output", sel_file_use), header=TRUE,
                        stringsAsFactors = FALSE,
                        as.is=T,
                        check.names = FALSE)
if(trial_set_number == 1){
  trial_tidy_all = trial2_tidy
}

trial_tidy_all <- trial1_tidy %>% bind_rows(trial2_tidy)
...
First group is compound by "202271DVGST_ciat", "202151DVGST_ciat", "202177DVGST_momi", "202176DVGST_repe"


```{r}
if (group == "group1") {
  trial_tidy_all <- trial_tidy_all %>% filter(trial_name %in% c("202151DVGST_ciat", "202176DVGST_repe", "202177DVGST_momi", "202271DVGST_ciat"))
...
```{r}
if(group == "group2"){ 
trial_tidy_all <- trial_tidy_all %>% filter(trial_name %in% c("202257DVGST_momi", "202256DVGST_stom"))
}else {
  print('We are working on group 1')
}
```
...
my_dat <- my_dat[!my_dat$trial_name %in% c("202151DVGST_ciat", "202271DVGST_ciat", "202258DVGST_ciat"), ]
...
shared_genos_2021 <- check_connectivity(
    ...
    trial = "trial_name",
    response = "germination_perc",
    all = TRUE,
    return_matrix = FALSE
  )
shared_genos_2021 <- shared_genos_2021 %>% distinct(accession_name) %>% 
  pull(accession_name)
...
shared_genos_2022 <- shared_genos_2022 %>% distinct(accession_name) %>% 
  pull(accession_name)
}
...
## 202151DVGST_ciat Palmira trial

```{r}

PALMIRA_51 <- trial_rm_sd %>% filter(trial_name %in% c("202151DVGST_ciat")) %>% 
  select(accession_name, any_of(trait_ideal))

# remove empty cols
PALMIRA_51 <- PALMIRA_51[, colSums(is.na(PALMIRA_51)) < nrow(PALMIRA_51)]

# extract reps in palmira
testigos_palmira_51 <- PALMIRA_51 %>% count(accession_name) %>% filter(n > 1) %>% pull(accession_name)

# calculate mean of checks in Palmira

palmira_51_checks_summary <- PALMIRA_51 %>% filter(accession_name %in% testigos_palmira_51) %>% 
  group_by(accession_name) %>% 
  summarise(across(where(is.numeric), mean))

# remeve checks from original data and bind their mean values

one_rep_palmira_51 <- PALMIRA_51 %>% filter(!accession_name %in% testigos_palmira_51) %>% 
  bind_rows(palmira_51_checks_summary)

var_ciat_51 <- colnames(one_rep_palmira_51)[-1] 

# paste BLUEs_ word to each trait of one_rep_palmira

colnames(one_rep_palmira_51) <- 
  c("accession_name", paste("BLUEs", colnames(one_rep_palmira_51[-1]), sep = "_"))

# add palmira BLUEs_value to master file

master_data$BLUE_202151DVGST_ciat <- 
  one_rep_palmira_51 %>%  mutate(across(where(is.numeric), round, 2))

```

## 202258DVGST_ciat Palmira trial

```{r}

PALMIRA_58 <- trial_rm_sd %>% filter(trial_name %in% c("202258DVGST_ciat")) %>% 
  select(accession_name, any_of(trait_ideal))

# remove empty cols
PALMIRA_58 <- PALMIRA_58[, colSums(is.na(PALMIRA_58)) < nrow(PALMIRA_58)]

# extract reps in palmira
testigos_palmira_58 <- PALMIRA_58 %>% count(accession_name) %>% filter(n > 1) %>% pull(accession_name)

# calculate mean of checks in Palmira

palmira_58_checks_summary <- PALMIRA_58 %>% filter(accession_name %in% testigos_palmira_58) %>% 
  group_by(accession_name) %>% 
  summarise(across(where(is.numeric), mean))

# remeve checks from original data and bind their mean values

one_rep_palmira_58 <- PALMIRA_58 %>% filter(!accession_name %in% testigos_palmira_58) %>% 
  bind_rows(palmira_58_checks_summary)

var_ciat_58 <- colnames(one_rep_palmira_58)[-1] 

# paste BLUEs_ word to each trait of one_rep_palmira

colnames(one_rep_palmira_58) <- 
  c("accession_name", paste("BLUEs", colnames(one_rep_palmira_58[-1]), sep = "_"))

# add palmira BLUEs_value to master file

master_data$BLUE_202258DVGST_ciat <- 
  one_rep_palmira_58 %>%  mutate(across(where(is.numeric), round, 2))
```

## 202271DVGST_ciat Palmira trial

```{r}

PALMIRA_71 <- trial_rm_sd %>% filter(trial_name %in% c("202271DVGST_ciat")) %>% 
  select(accession_name, any_of(trait_ideal))

# remove empty cols
PALMIRA_71 <- PALMIRA_71[, colSums(is.na(PALMIRA_71)) < nrow(PALMIRA_71)]

# extract reps in palmira
testigos_palmira_71 <- PALMIRA_71 %>% count(accession_name) %>% filter(n > 1) %>% pull(accession_name)

# calculate mean of checks in Palmira

palmira_71_checks_summary <- PALMIRA_71 %>% filter(accession_name %in% testigos_palmira_71) %>% 
  group_by(accession_name) %>% 
  summarise(across(where(is.numeric), mean))

# remeve checks from original data and bind their mean values

one_rep_palmira_71 <- PALMIRA_71 %>% filter(!accession_name %in% testigos_palmira_71) %>% 
  bind_rows(palmira_71_checks_summary)

var_ciat_71 <- colnames(one_rep_palmira_71)[-1] 

# paste BLUEs_ word to each trait of one_rep_palmira

colnames(one_rep_palmira_71) <- 
  c("accession_name", paste("BLUEs", colnames(one_rep_palmira_71[-1]), sep = "_"))

# add palmira BLUEs_value to master file

master_data$BLUE_202271DVGST_ciat <- 
  one_rep_palmira_71 %>%  mutate(across(where(is.numeric), round, 2))
```
...
master_data$BLUP_BLUE_202256DVGST_stom %>% 
  ...
  labs(title = "yield_ha - 202256DVGST_stom") +
  theme_xiaofei()

ggsave(paste('images\\yield_ha - 202256DVGST_stom', trial_interest, Sys.Date(), ".png", sep = "_"),
...
met_results <- met_analysis(obj, 
                            filter_traits = trait_ideal[!trait_ideal %in% c("lodging1_3_6mon")],
                            h2_filter = 0.09,
                            progress = TRUE)
...
# plant_type
covcor_heat(matrix = met_results$VCOV$plant_type$CORR, size = 6, legend =c(0.6, 0.9)) +
  theme(
  axis.text.y = element_text(size = 14),
  axis.text.x = element_text(size = 14))
...
# height_1st_branch
covcor_heat(matrix = met_results$VCOV$height_1st_branch$CORR, size = 6, legend =c(0.6, 0.9)) +
  theme(
  axis.text.y = element_text(size = 14),
  axis.text.x = element_text(size = 14))
...
# put BLUES from 202151DVGST_ciat data set together with the rest.
...
# put BLUES from 202271DVGST_ciat data set together with the rest.
blue_MET_palmira_71 <- master_data$BLUE_202271DVGST_ciat
    
  
  colnames(blue_MET_palmira_71)[-1] <-
    c(paste(colnames(blue_MET_palmira_71)[-1], "202271DVGST_ciat", sep = "_")) 
  

BLUEs_BLUPs <- 
  BLUEs_BLUPs %>% left_join(blue_MET_palmira_71, by = c("genotype" = "accession_name"))


# put BLUES from 202258DVGST_ciat data set together with the rest.
blue_MET_palmira_58 <- master_data$BLUE_202258DVGST_ciat
    
  
  colnames(blue_MET_palmira_58)[-1] <-
    c(paste(colnames(blue_MET_palmira_58)[-1], "202258DVGST_ciat", sep = "_")) 
  
  #BLUEs_branch_angle_202176DVGST_repe
  
BLUEs_BLUPs <- 
  BLUEs_BLUPs %>% left_join(blue_MET_palmira_58, by = c("genotype" = "accession_name"))
...
variables <- unique(c(variables, var_ciat_51, var_ciat_58, var_ciat_71))
...
BLUEs_BLUPs <- BLUEs_BLUPs %>% 
  select(genotype, contains(c("DM_gravity", "DM_nirs", "WAB")), everything())
...
names(master_data)[19] <- "BLUP_BLUE_germ_number_plot"
write.xlsx(master_data, file = meta_file_name)
