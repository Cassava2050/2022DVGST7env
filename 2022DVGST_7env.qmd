...
## save Xiaofei thrips1_5 from 202256_momi

```{r}

thrips <- read.delim("clipboard", header = T, check.names = F) %>% 
  select(plot_name, accession_name, plot_number, contains("thrips"))

thrips <- thrips %>% rename(thrips1_5 = `thrips severity : visual 1-5|CO_334:0002084`)
thrips$plot_number <- as.numeric(thrips$plot_number)

trial_tidy$plot_number <- as.numeric(trial_tidy$plot_number)


trial_tidy <- trial_tidy %>% filter(!trial_name == "202257DVGST_momi") %>% 
  bind_rows(trial_tidy %>% filter(trial_name == "202257DVGST_momi") %>% 
  select(-c(plot_name, thrips1_5)) %>% 
  left_join(thrips %>% select(plot_name, plot_number, thrips1_5), 
            by = c("plot_number" = "plot_number"))) 
```
...
First group is compound by "202151DVGST_ciat", "202176DVGST_repe", "202177DVGST_repe"


```{r eval=FALSE}
if (group == "group1") {
  trial_tidy_all <- trial_tidy_all %>% filter(trial_name %in% c("202151DVGST_ciat", "202176DVGST_repe", "202177DVGST_momi"))
} else {
...
Second group is compound by "202256DVGST_stom", "202257DVGST_stom"

```{r eval=FALSE}
if(group == "group2"){ 
trial_tidy_all <- trial_tidy_all %>% filter(trial_name %in% c("202256DVGST_stom", "202257DVGST_momi"))
}else {
...
```{r eval=FALSE}
accession_wrong <- trial_rm_sd %>% 
  filter(trial_name == "202257DVGST_momi", col_number %in% c(1:18), row_number %in% c(1:14)) %>% 
  select(accession_name) %>% pull()

accession_remove <- c("CM4919-1_is_Veronica", "TAI8_is_TAI")

reps_trips <- trial_rm_sd %>% filter(trial_name == "202257DVGST_momi", check_test == "test", 
                  !accession_name %in% accession_remove, accession_name %in% accession_wrong) %>% 
  select(check_test, accession_name, rep_number, thrips1_5) %>%
  pivot_wider(names_from = rep_number, values_from = thrips1_5) %>% 
  #arrange(col_number, row_number) %>% 
  rename(rep_1 = `1`, 
         rep_2 = `2`)

issue_genos <- reps_trips %>% pull(accession_name)

library(DataEditR)

#reps_trips_edit <- 
#data_edit(reps_trips) # pivot_longer results and put the thrips title, 
# then pull the genos and filter from original data and join with rep_numer and accession_name


reps_trips_edit <- read_csv("data/20230531-data.csv") # load fixed data

reps_trips_edit_long <-  # pivot longer fixed data
  reps_trips_edit %>% rename(`1` = rep_1, `2` = rep_2) %>% 
  pivot_longer(cols = c("1", "2"), names_to = "rep_number", values_to = "thrips1_5") 

reps_trips_edit_long %>% unite("accession_key", accession_name:rep_number, remove = T)

data_fixed <-
  trial_rm_sd %>% 
  filter(trial_name == "202257DVGST_momi", accession_name %in% issue_genos) %>% 
  select(plot_name, accession_name, rep_number) %>% 
  unite("accession_key", accession_name:rep_number, remove = F) %>% # mutate key column
  left_join(reps_trips_edit_long %>% 
              unite("accession_key", accession_name:rep_number, remove = T) %>% 
  select(-check_test), by = c("accession_key" = "accession_key")) %>% 
  select(plot_name, thrips1_5)
  
# replace above values into original data

col_row_data_fixed <- trial_rm_sd %>% filter(trial_name == "202257DVGST_momi", 
                  accession_name %in% issue_genos) %>% 
  select(-thrips1_5) %>% 
  left_join(data_fixed, by = "plot_name")

 data_momi_fixed <- trial_rm_sd %>% 
  filter(trial_name == "202257DVGST_momi", 
         !plot_name %in% col_row_data_fixed$plot_name) %>% 
   bind_rows(col_row_data_fixed)
```


Binding fixed data to tidy_data

```{r}
trial_rm_sd <- trial_rm_sd %>% filter(!trial_name == "202257DVGST_momi") %>% 
  bind_rows(data_momi_fixed)
```
...
There are a lot missing data, especially in rep_2. This may be the cause of no fitted any model. Several missing data. There 54% of missing data
...
reps_trips <- my_dat %>% filter(trial_name == "202257DVGST_momi", check_test == "test", 
...
reps_trips %>% select(rep_1, rep_2) %>% 
  summarise_all(~sum(is.na(.))/sum(nrow(reps_trips))) %>% # empty rows
  mutate(NA_rep_1_perc = rep_1 *100,
         NA_rep_2_perc = rep_2 *100) %>% 
  add_column(var = "thrips1_5")

gg_miss_var(reps_trips %>% select(rep_1, rep_2), show_pct = TRUE) +
  labs(x = "reps", title = "thrips1_5 missing values (202257DVGST_momi)")
```
...
my_dat57 <- my_dat %>% filter(trial_name == "202257DVGST_momi") %>% 
...
```{r eval=FALSE}
blupDF_kp <- blupDF_kp %>% 
  filter(accession_name %in% shared_genos)
```
