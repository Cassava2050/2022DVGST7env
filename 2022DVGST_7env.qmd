...
trial_standard <- trial_standard %>% 
  mutate(use_harvest_date = 
           ifelse(use_trial_name == "202257DVGST_momi", "2023-April-17", use_harvest_date))
```
...
my_dat <- my_dat[my_dat$trial_name == "202257DVGST_momi", ]
...
master_data$BLUP_BLUE_202257DVGST_momi %>% 
  select(genotype, BLUEs_branch_number, BLUPs_branch_number) %>% 
  ggplot(aes(x = BLUEs_branch_number, y = BLUPs_branch_number)) +
  geom_point() 

model_branch_number <- lm(BLUEs_branch_number ~ BLUPs_branch_number, data = master_data$BLUP_BLUE_202257DVGST_momi )

summary(model_branch_number)
model_branch_number$coefficients[1]
model_branch_number$coefficients[2]
```
## Single heritability
...
master_data$BLUE_202151DVGST_ciat <- one_rep_palmira %>%  mutate(across(where(is.numeric), round, 2))
...
names(master_data)[16] <- "BLUP_BLUE_germ_number_plot"
names(master_data)[25] <- "BLUP_BLUE_root_commercial"
names(master_data)[42] <- "stability_germ_number_plot"
names(master_data)[51] <- "stability_root_commercial"
