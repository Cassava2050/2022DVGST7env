...
my_dat <- my_dat[!my_dat$trial_name %in% c("202151DVGST_ciat", "202271DVGST_ciat"), ]
...
master_data$BLUE_202171DVGST_ciat <- 
  one_rep_palmira_71 %>%  mutate(across(where(is.numeric), round, 2))

# fix name of 202271

grep("^BLUE_202171DVGST_ciat", names(master_data))
names(master_data)[8] <- "BLUE_202271DVGST_ciat"
```
...
names(master_data)[18] <- "BLUP_BLUE_germ_number_plot"
write.xlsx(master_data, file = meta_file_name)

grep("^BLUP_BLUE_root_number_commercial", names(master_data))
names(master_data)[27] <- "BLUP_BLUE_root_commercial"
write.xlsx(master_data, file = meta_file_name)
```
